...
## 2026-03-20 - Kirilma Niyeti ve Bagimsiz Pattern Ogrenimi (Append-Only)

Bu surumde ogrenilen ve uygulanan noktalar:
1. Oyuncu niyetini sadece puan farkindan okumak yetersiz; ozel tas verimi, toplam-9 niyeti ve setup->getiri zinciri birlikte izlenmeli.
2. Hamle oncesi aday degerlendirme snapshot'u (`candidate_snapshot`) olmadan karar gerekcesi analizi eksik kaliyor.
3. Gelecek tas baglami loglarda acik tutulmali (`future_numbers`) yoksa planlama niyeti analizi zayifliyor.
4. Oyun disi surekli tarama (PatternWatch) canli oyundaki sapmalari daha erken yakaliyor.
5. H/J dashboard'ta yorum sutunu, salt sayi gostermekten daha degerli; operatore dogrudan aksiyon cagrisi veriyor.

Yeni ajan etkisi:
1. `breakpoint_agent.py`: 10/20/30 penceresinde kirilma anini ve niyet degisimini cikarir.
2. `pattern_watch_agent.py`: tum log havuzunu oyundan bagimsiz tarar, warning sinyallerini guidance'a ekler.
