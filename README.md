...
## 2026-03-20 - v3.10.0 Kırılma Analizi + Sürekli Pattern İzleme (Append-Only)

### Yeni AI modulleri (TR)
- `breakpoint_agent.py`: 10/20/30 tur pencerelerinde oyuncu kirilma anlarini, ozel tas mantigini, toplam-9 niyetini ve gelecek tas planlamasini cikarir.
- `pattern_watch_agent.py`: oyundan bagimsiz sekilde tum log havuzunu surekli tarar, pattern sapmalarini ve hedef kolon uyarilarini canli guidance katmanina ekler.

### Yeni loglama alanlari (TR)
- Hamle oncesi aday degerlendirme snapshot'u loglara eklendi (`candidate_snapshot`).
- Gelecek tas baglami daha acik izlenebilir hale getirildi (`future_numbers`).

### H/J analiz paneli (TR)
- Analiz paneli artik hem `H` hem `J` ile acilabilir.
- Dashboard tablosuna su metrikler eklendi:
	- Ozel Tas Mantik Skoru
	- Toplam-9 Niyet Orani
	- Gelecek Tas Plan Orani
	- Gelecek Veri Kapsami
	- PatternWatch metrikleri (ozel tas verim, sum9 odak, gelecek plan)
